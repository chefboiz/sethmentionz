...
def _momentum_score(current_price: float, old_price: float | None, mid: str) -> float:
    ...
    # Map price-change to [0, 1]: 0% change → 0.5; +100% → 1.0; -100% → 0.0
...
def run_longshot_score() -> None:
    ...
    for m in markets:
        ...
        # Identify cheap side — must be ≤ LONGSHOT_MAX_PRICE and non-zero
        if 0 < yes_price <= LONGSHOT_MAX_PRICE:
            cheap_side, cheap_price, expensive_price, cheap_idx = 'yes', yes_price, no_price, 0
        elif 0 < no_price <= LONGSHOT_MAX_PRICE:
            cheap_side, cheap_price, expensive_price, cheap_idx = 'no', no_price, yes_price, 1
        else:
            skipped_price += 1
            continue
        ...
        # Liquidity gate: fetch order book for cheap side
        book = fetch_book(clob_ids[cheap_idx])
        if not book:
            continue
        ...
        # Use book's best ask as more accurate cheap_price
        asks = book.get('asks', [])
        if asks:
            cheap_price = asks[0]['price']

        # Snapshot for future momentum calculations
        ...
        db.execute("""
            INSERT INTO mention_longshot_candidates (
                ...
                composite_score, subject, last_scored_at
            ) VALUES (
                ...
                %(composite_score)s, %(subject)s, NOW()
            )
        ...
        scored += 1
        log.info('  %s | %s @%.2f  comp=%.3f (m=%.2f v=%.2f t=%.2f)',
                 mid[:14], cheap_side.upper(), cheap_price, composite,
                 mom_score, vol_score, t_score)

    log.info('=== Longshot score done | scored: %d  skipped_depth: %d  no_cheap_side: %d ===',
             scored, skipped_depth, skipped_price)
